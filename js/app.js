// Forscher-Akademie — Übertritt-Ferienkurs — Navigation & Rendering
...
const STORAGE_KEY = "forscherAkademieUebertrittProgress_v1";
...
function loadProgress() {
  try {
    ...
    return { leoXP: 0, starDone: {}, trackerDone: {} };
  }
}
...
function saveProgress() {
  ...
  } catch (e) { /* Speicher evtl. voll/gesperrt — kein Absturz */ }
}

const PROGRESS = loadProgress();
const SESSION = { leoXP: PROGRESS.leoXP };

function updateSessionCounter() {
  const el = document.getElementById("sessionCounter");
  if (!el) return;
  el.innerHTML = `🦁 Kadett: <strong>${SESSION.leoXP} XP</strong>`;
}
...
function renderMaterials() {
  ...
  const header = `
    ...
    <p class="page-subtitle">Alles, was du vorab ausdrucken oder besorgen kannst — schau hier immer schon am Vortag rein.</p>
...
function renderExcursions() {
  ...
  return `
    ...
    an die Wand hängen, damit sich dein Kind schon darauf freuen kann!</p>
    ${cards}
  `;
}
